...
Java 싱글톤 구현은 모두 같은 접근점을 제공하지만, 인스턴스를 만드는 시점과 동시 접근을 제어하는 방식이 다르다. 단순 지연 생성은 멀티스레드 환경에서 안전하지 않고, `synchronized` 접근자와 DCL은 안전성을 얻는 대신 각각 반복 잠금 비용과 구현 복잡도를 부담한다. 일반적인 선택은 클래스 형태와 지연 초기화가 필요하면 Lazy Holder, enum 형태가 자연스러우면 enum이며, 공유 상태 자체의 동시성은 별도로 설계해야 한다.
...
- **지연 초기화**: 싱글톤 타입이 초기화된 뒤에도 실제 인스턴스를 요청할 때까지 생성을 미룰 수 있는지 확인한다.
...
| 3. 정적 `final` 즉시 생성 | 싱글톤 클래스 초기화 | 보장 | 불가능 | 사용하지 않아도 클래스 초기화와 함께 생성될 수 있음 |
| 4. 정적 초기화 블록 | 싱글톤 클래스 초기화 | 보장 | 불가능 | 이른 생성과 다단계 초기화 코드의 복잡도 |
...
| 7. enum | enum 클래스 초기화 | 보장 | 불가능 — enum 클래스 초기화 시 생성 | enum 형태의 API와 상속 제한을 수용해야 함 |
...
3번은 선언과 동시에 `static final` 필드에 할당하며 초기화가 단순할 때 적합하다. 4번은 같은 즉시 생성 계열이지만 정적 블록 안에서 여러 단계의 준비나 예외 변환을 할 수 있고, 두 방식 모두 인스턴스를 쓰지 않아도 싱글톤 클래스가 초기화되면 생성 비용을 낸다. enum도 enum 클래스 초기화 과정에서 상수를 생성하므로 같은 기준에서는 지연 초기화가 아니다.
...
Java Memory Model에서 `volatile` 필드에 대한 쓰기는 이후 같은 필드를 읽는 동작보다 happens-before이다. 생성자 실행 뒤 `instance`에 참조를 쓰고 다른 스레드가 그 값을 읽으면, 프로그램 순서와 이 happens-before 관계를 통해 초기화 중 기록한 상태도 함께 보이게 된다. 이는 `volatile`이 CPU 캐시를 단순히 사용하지 않게 만든다는 뜻이 아니라, 해당 필드 접근 사이의 가시성과 순서를 언어 수준에서 보장한다는 뜻이다.
