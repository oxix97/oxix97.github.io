...
- **지연 초기화**: 사용하지 않는 인스턴스의 생성 비용을 피할 수 있는지 확인한다.
...
| 7. enum | enum 클래스 초기화 | 보장 | 클래스 초기화 시점까지 | enum 형태의 API와 상속 제한을 수용해야 함 |
...
3번은 선언과 동시에 `static final` 필드에 할당하며 초기화가 단순할 때 적합하다. 4번은 같은 즉시 생성 계열이지만 정적 블록 안에서 여러 단계의 준비나 예외 변환을 할 수 있고, 두 방식 모두 인스턴스를 쓰지 않아도 싱글톤 클래스가 초기화되면 생성 비용을 낸다.
