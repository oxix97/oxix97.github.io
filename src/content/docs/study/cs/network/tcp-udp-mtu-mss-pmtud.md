...
먼저 애플리케이션에 필요한 전달 방식을 고른다. 다음으로 한 패킷에 담을 수 있는 크기를 계산한다. 마지막으로 경로 중간에서 그 크기의 패킷이 통과하지 못할 때 어떤 신호로 조정하는지 확인한다.
...
TCP는 연결을 설정할 때 두 종단이 초기 시퀀스 번호를 독립적으로 선택해 교환한다. 이후 바이트마다 시퀀스 공간을 부여하고 누적 ACK로 다음에 기대하는 바이트를 알린다. 이것이 순서를 맞추는 기준이다.

손실 복구는 송신 측이 맡는다. 타임아웃이나 중복 ACK 같은 신호로 손실을 감지해 재전송한다. TCP 체크섬은 받은 세그먼트의 오류를 검출한다.

전송량을 제한하는 장치도 있다. 수신 윈도는 수신자가 받을 수 있는 양을 알리고, 혼잡 제어는 네트워크 상황에 맞춰 전송량을 조절한다.
...
그림에서는 IP 패킷 전체와 TCP 데이터 부분을 구분한다. 헤더 옵션이 없는 예시라는 조건도 함께 읽는다.
...
MSS 광고값을 계산하는 기준과 실제 패킷에 담는 데이터 길이는 구분해야 한다. 아래 계산은 먼저 광고값을 구하고, 이어 옵션이 있을 때의 데이터 길이를 설명한다.
...
그림을 왼쪽부터 읽으면 1500바이트짜리 IP 패킷에서 IPv4 헤더 20바이트를 빼고, TCP 헤더 20바이트를 다시 뺀 나머지가 1460바이트다. 이 숫자는 그림의 조건에서 나온 결과다.
...
큰 패킷이 막힌 뒤 피드백이 송신자에게 돌아오는 방향을 따라가 보자.
...
이 흐름은 ICMP 피드백이 돌아오는 정상 사례다. 피드백이 차단되면 송신자는 같은 방식으로 크기를 조정하지 못할 수 있다. 다음의 PLPMTUD는 패킷화 계층의 프로브와 확인으로 이 의존성을 줄인다.
...
앞 그림의 PMTUD는 너무 큰 패킷을 버린 라우터가 오류 메시지를 돌려주는 흐름이다. 그 메시지가 돌아오지 않아도 크기를 시험할 방법이 필요한 경우를 이어서 보자.
