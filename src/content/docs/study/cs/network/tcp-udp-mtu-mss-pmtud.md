...
TCP는 연결을 설정할 때 양쪽의 초기 상태와 시퀀스 번호를 맞추고, 전송한 바이트에 시퀀스 번호를 붙여 수신 순서를 식별한다. 수신 측 ACK는 다음에 기대하는 바이트 번호를 알려 주며, 송신 측은 ACK가 오지 않거나 손실을 감지하면 데이터를 재전송한다. TCP 체크섬은 세그먼트의 오류를 검출하며, 시퀀스 번호와 재전송은 그 오류나 손실을 복구하는 흐름의 일부다.
