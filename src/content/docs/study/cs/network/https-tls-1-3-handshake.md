...
처음에는 세 질문만 잡고 읽는다. **서버가 누구인지 어떻게 확인하는가? 두 종단이 같은 비밀을 어떻게 만드는가? 이후 HTTP 데이터는 무엇으로 보호하는가?** 인증서·ECDHE·AEAD는 이 세 질문의 서로 다른 답이다.
...
핸드셰이크에서 따라갈 일은 세 가지다. 사용할 버전과 암호 조합을 정하고, 서버를 인증하며, 이후 통신에 쓸 키를 만든다. 다음 흐름은 인증서로 서버를 인증하는 일반적인 전체 1-RTT 핸드셰이크다. HelloRetryRequest, 클라이언트 인증, 재개 연결은 우선 제외한다.

아래 순서도에서는 왼쪽의 클라이언트와 오른쪽의 서버 사이를 오가는 메시지만 먼저 따라간다. `ClientHello`와 `ServerHello`는 협상과 공개 key share 교환, `Certificate`와 `CertificateVerify`는 서버 인증, `Finished`는 앞선 핸드셰이크의 확인에 해당한다.
...
공유 비밀을 그대로 하나의 세션키로 쓰지는 않는다. TLS 1.3 키 스케줄은 HKDF-Extract와 HKDF-Expand-Label을 사용해 핸드셰이크 트래픽 비밀, 애플리케이션 트래픽 비밀, 실제 레코드 보호 키와 IV를 단계별로 파생한다.

클라이언트 송신과 서버 송신의 비밀도 분리한다.
...
네트워크로 교환하는 값과 각 종단 안에서 계산하는 값을 나누어 보자.

그림에서 화살표로 건너가는 것은 공개 key share다. 공유 비밀과 트래픽 키는 두 종단이 각자 계산하며 그대로 전송하지 않는다. 인증서의 장기 개인키도 이 키 계산 결과를 대신하지 않는다.
...
인증서의 장기 키는 서명으로 신원을 증명한다. 그림에서 만드는 공유 비밀과 트래픽 키는 그 인증서 키를 그대로 복사한 값이 아니다.
...
여기까지는 처음 연결할 때의 기본 흐름이다. 0-RTT는 이전 연결의 재개 정보를 가진 클라이언트가 **서버 응답 전에 데이터를 보내는 예외적인 흐름**이므로, 기본 핸드셰이크와 구분해 읽는다.
