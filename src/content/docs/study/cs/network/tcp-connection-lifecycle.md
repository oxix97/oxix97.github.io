...
처음에는 상태 이름보다 두 방향만 본다. 연결할 때는 양쪽이 각자의 시작 번호를 알리고, 종료할 때는 각자의 송신 방향을 따로 닫는다. 아래 그림의 `x`는 연결을 시작한 쪽, `y`는 응답한 쪽이 정한 번호다.
...
첫 번째 화살표에서 시작 측의 `x`를, 두 번째에서 응답 측의 `y`를 찾는다. 세 번째 화살표까지 오면 양쪽 모두 상대의 시작 번호를 확인했다.
...
정상 종료 그림에서 먼저 **FIN을 받은 쪽**과 **마지막 ACK를 보낸 쪽**을 찾는다. 전자는 자신의 송신을 닫기 전 `CLOSE_WAIT`에 머물고, 후자는 마지막 ACK 뒤 `TIME_WAIT`에 머문다. 동시 종료 같은 다른 흐름은 앞서 설명한 예외를 따른다.
