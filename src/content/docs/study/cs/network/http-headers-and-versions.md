...
먼저 요청 한 개가 어떤 부분으로 이루어졌는지 본다. 그다음 같은 연결에서 요청 두 개를 보낼 때 앞 응답이 늦어지면 어떻게 되는지 버전별로 비교한다. 메시지의 의미와 전달 방식은 서로 다른 질문이다.
...
코드에서 `POST /orders HTTP/1.1`은 시작줄이고, 그 아래 세 줄은 필드다. **빈 줄이 필드의 끝**을 표시하며 `{`로 시작하는 줄부터 본문이다. 응답 예시도 같은 경계로 읽으면 된다.
...
두 요청 A와 B를 연달아 보냈다고 하자. B를 빨리 처리해도 A의 응답이 먼저 나가야 한다면 B는 기다린다. 이렇게 앞의 작업이 뒤의 작업을 막는 현상을 Head-of-Line(HOL) Blocking이라고 부른다.
...
그림의 세 경우에서 요청 A와 B는 같다. HTTP/1.1에서는 응답 순서, HTTP/2에서는 공유 TCP 연결의 손실, HTTP/3에서는 손실이 난 QUIC 스트림의 범위를 각각 따라가면 된다.
