...
먼저 계층마다 어떤 책임과 PDU를 사용하는지 위에서 아래로 읽는다.
...
같은 데이터도 어느 계층에서 보는지에 따라 이름이 달라진다. 아래에서는 각 계층의 책임을 먼저 설명하고, 표에서 프로토콜과 PDU를 비교한다.

뒤에서 사용할 예시는 이미 연결된 브라우저가 `GET /articles`를 보내는 상황이다. 브라우저가 만든 HTTP 바이트가 아래 계층으로 내려가며 이름과 포장 방식이 어떻게 바뀌는지 따라가면 된다. PDU는 **각 계층이 다루는 데이터 단위**를 뜻한다.
...
PDU 이름은 어느 계층의 관점에서 데이터를 부르는지 나타낸다. RFC 9293은 애플리케이션 바이트 스트림이 TCP 세그먼트로 나뉘고 각 세그먼트가 IP 데이터그램에 실린다고 설명한다.

TCP는 애플리케이션 메시지 경계를 보존하지 않는다. 한 HTTP 메시지가 여러 세그먼트로 나뉠 수 있고, 애플리케이션의 여러 쓰기 결과가 TCP에서 합쳐질 수도 있다.
...
송신 측은 아래 계층으로 내려가고 수신 측은 위 계층으로 올라간다. 양쪽의 처리 순서를 비교해 보자.
...
그림은 종단의 처리 순서를 보여 준다. 중간 라우터가 만드는 새 링크 프레임까지 하나의 프레임이 계속 이동하는 것으로 읽으면 안 된다.

`GET /articles`를 담은 IP 패킷이 라우터를 지나면 라우터는 목적지 IP를 보고 다음 홉을 고른다. 그다음 **다음 링크에 맞는 새 프레임**에 IP 패킷을 담는다. 그림의 한 프레임이 목적지까지 그대로 이동하는 것은 아니다.
...
브라우저가 이미 설정된 TCP 연결에서 HTTP/1.1 `GET /articles`를 보낸다고 가정하자. DNS 조회, TCP 연결, TLS 협상은 예시에서 생략한다.

HTTP 메시지의 바이트는 TCP가 정한 크기의 세그먼트 하나 이상에 담긴다. 각 세그먼트는 IP 패킷에, IP 패킷은 현재 링크의 프레임에 담긴다. HTTP/2도 TCP 바이트 스트림 위에 자체 바이너리 프레임을 두므로 HTTP/2 프레임과 TCP 세그먼트의 경계가 일치한다고 보장할 수 없다.
