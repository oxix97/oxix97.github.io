...
저장할 값을 고를 때는 먼저 세 가지를 묻는다. **다음 방문에도 필요한가? 탭마다 달라야 하는가? 서버가 요청마다 읽어야 하는가?** 예를 들어 테마 설정, 탭별 작성 초안, 로그인 세션 식별자는 답이 서로 다르다.
...
테마 설정을 저장한다고 이전 HTML 응답이 재사용되지는 않는다. 반대로 캐시된 이미지를 다시 표시한다고 로그인 상태가 이어지는 것도 아니다. 무엇을 저장하는지부터 구분하면 뒤의 세 저장 수단을 비교하기 쉽다.
...
처음 질문으로 돌아가 보자. 다음 방문의 테마는 로컬스토리지, 탭마다 다른 작성 초안은 세션스토리지, 서버가 매 요청 확인하는 작은 세션 식별자는 조건에 맞는 쿠키가 후보가 된다. 민감 정보와 브라우저 정책은 표 아래의 제약도 확인해야 한다.
