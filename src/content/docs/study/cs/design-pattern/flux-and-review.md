...
Flux는 상태 변경 요청이 여러 Model과 View 사이를 우회하지 않고 일정한 경로를 따르게 하는 UI 애플리케이션 아키텍처 패턴이다. 핵심 순환은 **`Action → Dispatcher → Store → View → Action`**이며, 상태를 바꾸는 입구와 순서가 드러나므로 한 사용자 행동의 결과를 추적하기 쉬워진다.
...
여러 화면이 같은 Model을 관찰하고 각 View의 사용자 이벤트가 다시 Model을 바꾸는 대규모 MVC 변형을 생각해 보자. 메시지 하나를 읽음 처리했을 때 메시지 목록, 안 읽은 개수 배지, 알림 패널이 각자의 표시 상태를 바꾸고 그 변경이 또 다른 갱신을 일으키면 하나의 입력이 어디까지 전파되는지 알기 어렵다. 강의에서는 이런 Model·View 관계의 증가를 Flux가 등장한 배경으로 설명한다.
...
강의는 Action, Dispatcher, Store, View의 네 역할과 Action type에 따른 처리를 핵심으로 다룬다. 전통적인 Facebook Flux를 더 엄밀히 보면 Dispatcher는 복잡한 분기 로직을 소유하는 Controller라기보다 등록된 Store 콜백 전체에 Action을 방송하는 중심 허브에 가깝다. 각 Store가 type을 보고 자신에게 관련된 Action인지 판단하며, Redux 같은 파생 구현은 Dispatcher와 Store 갱신 규칙을 reducer와 `dispatch`로 다르게 구성할 수 있다.
...
실패 이유나 재시도 버튼처럼 화면에 새로운 표현이 필요하면 View도 수정된다. Flux는 변경을 제거하는 것이 아니라, 변경 원인이 Action·Store·effect·View 중 어느 경계에 속하는지 드러낸다.
...
MVC는 표현·입력·도메인 책임을 나누는 데 초점이 있고, Flux는 공유 상태가 변경되는 경로를 통제하는 데 초점이 있다. 따라서 서버 참조 위주의 간단한 CRUD 화면은 MVC만으로도 충분할 수 있고, 여러 View가 공유 상태를 동시에 표현하고 낙관적 갱신·롤백·서버 이벤트를 같이 처리하면 Flux 계열 구조의 비용이 정당화될 수 있다. 하나의 애플리케이션이 외부 HTTP 요청은 MVC Controller로 받고 복잡한 클라이언트 상태는 Flux 계열로 관리하듯 두 선택을 다른 경계에서 함께 쓸 수도 있다.
...
Flux의 장점은 변경 입구와 순서가 Action 흐름으로 드러난다는 점이다. 같은 Action과 이전 상태를 재현해 Store 상태 전이를 검증하기 쉬운 구조로 만들 수 있고, 버그가 나면 View에서 직접 setter를 찾는 대신 Action 기록을 시작으로 역추적할 수 있다. 여러 View가 공유하는 상태도 Store의 하나의 원본에서 파생하면 화면별 값이 서로 엇갈릴 가능성을 줄인다.
