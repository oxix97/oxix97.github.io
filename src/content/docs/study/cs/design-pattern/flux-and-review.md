...
메시지를 읽으면 목록의 ‘안 읽음’ 표시가 사라지고, 안 읽은 개수 배지도 줄어야 한다. 목록과 배지가 각자 값을 바꾸면 둘이 서로 다른 상태를 보여 줄 수 있다. Flux가 상태 변경을 일정한 경로로 모으는 방법을 살펴보고, 연재에서 다룬 패턴을 해결하는 문제별로 복습한다.

## 핵심 요약

Flux는 여러 화면이 공유하는 상태를 일정한 경로로 바꾸는 UI 애플리케이션 아키텍처 패턴이다. 화면에서 상태를 제각각 바꾸는 대신, ‘메시지를 읽었다’는 요청을 보내고 정해진 곳에서 상태를 바꾼다. 화면은 그 결과를 읽어 다시 표시한다.

핵심 순환은 **`Action → Dispatcher → Store → View → Action`**이다. 각 이름은 차례로 ‘무슨 일이 일어났는지’, ‘어디로 전달할지’, ‘상태를 어떻게 바꿀지’, ‘화면에 어떻게 보여 줄지’를 맡는다. 마지막 View에서 다음 Action으로 이어지는 것은 새로운 사용자 행동 등이 발생했을 때다.

흐름이 정해져 있으면 어떤 행동 때문에 값이 바뀌었는지 따라가기 쉽다. 대신 작은 화면에도 여러 구성 요소를 준비해야 하므로, 모든 화면에 필요한 구조는 아니다.
...
메시지 읽음 사례로 역할을 먼저 나누어 보자. Action은 ‘어느 메시지를 읽었는지’라는 요청이다. Dispatcher는 이 요청을 Store들에 전달하고, Store는 메시지의 읽음 여부 같은 상태를 바꾼다. View는 그 상태를 목록과 배지로 보여 준다.

표의 `type`은 요청 종류이고 `payload`는 그 요청에 필요한 데이터다. 예를 들어 읽음 처리 요청에는 메시지 식별자가 필요하다. 콜백은 다른 구성 요소가 정해진 시점에 호출하도록 등록해 둔 함수다.
...
그림에서 하나의 읽음 요청이 목록과 배지에 반영되는 과정을 위에서 아래로 따라가 보자. 여기서는 메시지 상태를 관리하는 Store 하나에서 두 화면이 값을 읽는 예를 사용한다.

<figure class="study-diagram study-diagram--pattern">
  <img src="/images/study/design-pattern/flux-read-message.svg" width="480" height="582" alt="사용자가 메시지 읽음을 선택하면 Action을 만들고 Dispatcher가 등록된 Store 콜백에 전달한다. Store가 상태를 갱신하고 View가 목록과 배지를 다시 표시한다. 다음 Action은 새로운 이벤트에서 시작한다." loading="lazy" decoding="async" />
  <figcaption>목록과 배지가 같은 상태를 읽는다. 화면을 다시 그리는 일만으로 다음 Action이 자동 발생하지는 않는다.</figcaption>
</figure>
...
화면 갱신 흐름을 이해했다면 서버 통신이 들어가는 위치를 보자. `dispatch`는 Action을 전달하는 동작이다. 전통적인 Flux에서 `dispatch(action)`과 그에 대응하는 Store 콜백·상태 변경은 **한 번의 dispatch 라운드 안에서 동기적으로** 완료된다고 가정한다. 그래야 순서 의존이 있는 Store도 먼저 갱신된 Store를 확인한 뒤 같은 라운드에서 결과를 계산할 수 있다.

서버 응답을 기다리는 작업은 언제 끝날지 알 수 없다. 이런 비동기 작업을 위의 한 번의 상태 갱신 구간에 섞으면 Action 순서와 실패 정책이 흐려진다. 네트워크·타이머·저장소 I/O는 Action creator, effect 계층, middleware 같은 바깥 경계에서 처리한다. 이후 `REQUESTED`, `SUCCEEDED`, `FAILED`처럼 결과를 다시 Action으로 표현한다. 구체적인 비동기 API는 Flux 계열 라이브러리마다 다르므로 해당 구현의 계약을 확인해야 한다.

### 새 요구 사항이 들어오면 어디가 바뀌는가

서버 응답을 기다리기 전에 화면을 먼저 바꾸는 방식을 낙관적 갱신이라고 한다. 사용자는 빠르게 결과를 보지만, 서버 저장에 실패하면 이전 상태로 되돌리는 롤백이 필요하다. effect 계층은 여기서 서버 통신 같은 바깥 작업을 맡는 코드를 뜻한다.

“메시지를 바로 읽음 표시하되, 서버 저장이 실패하면 안 읽음으로 되돌리고 오류를 보여 준다”는 요구를 추가해 보자.

<figure class="study-diagram study-diagram--pattern">
  <img src="/images/study/design-pattern/flux-rollback.svg" width="480" height="542" alt="읽음 요청 Action으로 화면에 먼저 반영한 뒤 effect에서 서버 저장을 요청한다. 실패 응답이 새 실패 Action으로 전달되면 Store가 이전 상태를 복구하고 목록과 배지가 함께 갱신된다." loading="lazy" decoding="async" />
  <figcaption>한 번의 읽음 요청에 대한 실패 사례다. 서버 통신은 상태 갱신 밖에서 수행하고 결과를 새 Action으로 전달한다.</figcaption>
</figure>

이 그림은 요청 하나가 실패하는 경우다. 같은 메시지의 요청이 겹치면 어떤 요청을 되돌릴지 식별하는 규칙도 필요하다. 아래 표에서는 이 처리를 어디에 작성해야 하는지 비교한다.
...
앞 글의 MVC는 데이터·화면·입력의 책임을 나눴다. Flux는 여러 화면이 공유하는 상태를 어떤 경로로 바꾸는지에 초점을 둔다. 표는 특정 구현의 우열보다 두 방식에서 살펴봐야 할 질문을 비교한다.
...
처음부터 표 전체를 외울 필요는 없다. 겪고 있는 상황과 가까운 질문을 먼저 고르면 된다.

- 객체를 만들거나 공유하는 방식이 문제인가? 싱글톤과 팩토리를 다시 본다.
- 목록의 저장 방식을 몰라도 하나씩 읽고 싶은가? 이터레이터를 본다.
- 결제처럼 같은 작업의 수행 방법을 바꾸고 싶은가? 전략 패턴을 본다.
- 상태가 바뀐 뒤 실행할 작업이 늘어나는가? 옵저버를 본다.
- 실제 대상에 접근하기 전에 정책을 적용해야 하는가? 프록시를 본다.
- 화면의 역할 분리나 여러 화면의 상태 갱신이 어려운가? MVC 계열과 Flux를 비교한다.
