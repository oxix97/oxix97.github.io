...
앞 글에서는 여러 기능이 같은 객체를 공유하는 구조를 살펴봤다. 그런데 두 요청이 동시에 처음 접근하면, 두 쪽 모두 새 객체를 만들려고 할 수 있다. 이 글은 Java 구현을 비교하는 심화 편이다. 먼저 중복 생성이 생기는 상황을 그림으로 보고, 일곱 방식이 언제 객체를 만들고 어떻게 보호하는지 비교한다.

## 핵심 요약

구현을 비교할 때는 두 질문에서 시작하면 된다.

- **언제 만드는가?** 클래스가 초기화될 때 만들 수도 있고, 실제로 객체를 요청할 때까지 미룰 수도 있다. 뒤의 방식을 지연 초기화라고 한다.
- **동시에 요청해도 안전한가?** 두 요청이 겹쳐도 하나만 만들고, 만들어진 객체를 다른 요청이 올바르게 볼 수 있어야 한다.

클래스 형태를 유지하며 생성을 미루려면 Lazy Holder를, enum 형태가 API에 맞으면 enum을 검토할 수 있다. `synchronized`와 DCL은 잠금을 사용하며, 특히 DCL은 추가로 메모리 모델을 이해해야 한다. 각 이름의 차이는 아래 표와 코드에서 확인한다.

어느 방식을 택해도 ‘객체를 하나만 만드는 것’과 ‘그 객체의 값을 여러 요청이 안전하게 바꾸는 것’은 별개다.

## 비교 기준

스레드는 프로그램 안에서 일을 진행하는 실행 흐름이다. 여러 스레드는 같은 코드에 동시에 접근할 수 있다. ‘아직 객체가 없으면 만든다’는 코드를 두 스레드가 실행한다고 해 보자.

<figure class="study-diagram study-diagram--pattern">
  <img src="/images/study/design-pattern/singleton-race.svg" width="480" height="484" alt="시간이 흐르며 스레드 A와 B가 모두 null을 확인한 다음 각자 다른 객체를 만드는 경쟁 상황. 가능한 실행 순서 하나를 나타낸다." loading="lazy" decoding="async" />
  <figcaption>없음을 확인하는 일과 새 객체를 만드는 일 사이에 다른 스레드가 들어올 수 있다.</figcaption>
</figure>

그림처럼 두 스레드가 모두 `null`, 즉 객체가 없다는 값을 읽으면 각각 생성할 수 있다. 확인과 생성을 하나로 보호하지 않았기 때문이다. 이렇게 실행 순서에 따라 결과가 달라지는 문제를 경쟁 조건이라고 한다. 잠금은 한 스레드가 보호된 코드를 실행하는 동안 다른 스레드가 그 구간에 들어오지 못하게 한다.

이 문제를 염두에 두고 다음 네 기준을 비교한다. 표의 스레드 안전성은 인스턴스 생성과 공개에 대한 것이며, 객체 안의 모든 작업을 보장한다는 뜻은 아니다.
...
처음에는 생성 시점과 스레드 안전성 열만 읽어도 된다. 1번은 보호가 없는 형태, 2·6번은 직접 잠금을 사용하는 형태, 3·4·5·7번은 Java의 클래스 초기화 규칙을 이용하는 형태로 묶어 보면 차이를 찾기 쉽다. 세부 비용은 각 방식의 설명을 읽은 뒤 다시 비교한다.
...
Holder는 공유 객체를 담아 두는 중첩 클래스다. 바깥 클래스가 초기화됐다고 안쪽 클래스까지 바로 초기화되는 것은 아니라는 점을 이용한다. 아래 그림은 실행 순서다.

<figure class="study-diagram study-diagram--pattern">
  <img src="/images/study/design-pattern/singleton-holder.svg" width="480" height="542" alt="AppSettings 초기화 시 Holder는 아직 초기화하지 않고, 최초 getInstance 호출에서 Holder.INSTANCE를 사용하면 Holder를 초기화하여 객체를 만든다. 이후에는 같은 객체를 반환한다." loading="lazy" decoding="async" />
  <figcaption>생성을 미루는 위치는 Holder이며, 최초 초기화의 동기화는 JVM이 맡는다.</figcaption>
</figure>

코드에서는 `new AppSettings()`가 바깥 클래스가 아니라 `Holder` 안에 있다는 점을 확인한다.
...
DCL은 Double-Checked Locking의 약자다. 이미 만들어졌는지 잠금 밖에서 한 번, 잠금 안에서 다시 한 번 확인한다. 이 절은 Java 동시성의 세부 동작을 다룬다. 처음 읽을 때는 ‘잠금은 중복 생성을 막고, `volatile`은 다른 스레드가 초기화 결과를 올바르게 보도록 한다’는 역할 차이에 집중하면 된다.
...
Java Memory Model은 여러 스레드가 값을 쓰고 읽을 때 무엇을 볼 수 있는지 정한 규칙이다. 여기서 `volatile` 필드에 대한 쓰기는 이후 같은 필드를 읽는 동작보다 happens-before이다. 이 관계는 단순히 시계상 먼저 일어났다는 뜻이 아니라, 앞선 쓰기의 결과가 뒤의 읽기에 보이도록 하는 보장이다.

생성자 실행 뒤 `instance`에 참조를 쓰고 다른 스레드가 그 값을 읽으면 초기화 중 기록한 상태도 함께 보인다. 이는 `volatile`이 CPU 캐시를 사용하지 않게 만든다는 뜻이 아니다. 해당 필드 접근 사이의 가시성과 순서를 언어 수준에서 보장한다는 뜻이다.
