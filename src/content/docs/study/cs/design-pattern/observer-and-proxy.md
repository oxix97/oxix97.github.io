...
주문 상태 변경 뒤 여러 후속 작업을 실행하는 문제와 결제 대상 앞에 정책을 끼우는 문제는 호출 흐름부터 다르다. 옵저버와 프록시가 만드는 두 간접 경계를 나란히 살펴본다.

## 핵심 요약

옵저버 패턴은 Subject의 상태 변경을 등록된 Observer 여러 개에게 통지한다. 이를 통해 발행자와 후속 처리를 분리한다. 프록시 패턴은 대상과 같은 계약을 제공하면서 접근 제어와 로깅 같은 정책을 적용한다. 두 패턴은 모두 간접 계층을 둔다. 하지만 여러 구독자에게 변화를 알리는 목적과 한 대상으로 가는 접근을 대리하는 목적은 다르다.
