...
주문이 배송되면 이메일과 문자를 보내고 싶다. 결제를 실행하기 전에는 권한을 확인하고 싶다. 앞의 문제는 여러 곳에 변화를 알리는 옵저버로, 뒤의 문제는 실제 대상 앞에서 요청을 확인하는 프록시로 살펴볼 수 있다.

## 핵심 요약

- 옵저버는 **하나의 상태 변경을 등록된 여러 구독자에게 알리는 구조**다. 주문은 상태가 바뀌었다고 알리고, 이메일과 문자 객체는 각자 후속 작업을 수행한다.
- 프록시는 **실제 대상과 같은 사용 방법을 제공하는 대리 객체**다. 결제 요청을 먼저 받아 권한을 확인한 뒤 실제 결제 객체로 전달할 수 있다.

옵저버는 알림이 여러 대상으로 퍼지고, 프록시는 대상에 가는 길에 대리자가 놓인다. 두 그림의 화살표를 먼저 비교한 뒤 코드에서 같은 역할을 찾아보자. 네트워크 프록시와 Spring AOP 절은 기본 구조를 이해한 다음 읽는 응용 설명이다.
...
옵저버 패턴에서 Subject는 변화가 일어나는 대상이고, Observer는 그 변화를 전달받는 구독자다. 이 예제의 Subject는 `Order`, Observer는 이메일과 문자 알림 객체다.

`Order`는 `OrderStatusObserver`라는 공통 알림 방법과 구독 목록만 안다. 상태를 바꾼 뒤 목록에 등록된 객체의 `update()`를 호출한다. 구독자가 주기적으로 주문에 변화가 있는지 물어보는 폴링과는 다르다.
...
이 구조가 격리하는 변화는 새 구독자 추가다. 반면 상태 이벤트 계약에 배송지 정보가 추가되면 계약과 모든 Observer가 영향을 받을 수 있다.

## Subject와 Observer의 흐름

먼저 구독자를 등록하고, 주문 상태를 바꾸고, 등록된 구독자에게 차례로 알린다. 아래 그림은 코드와 같은 등록 순서다. 화살표가 갈라져 있어도 동시에 실행한다는 뜻은 아니다.

<figure class="study-diagram study-diagram--pattern">
  <img src="/images/study/design-pattern/order-observers.svg" width="480" height="490" alt="이메일과 SMS를 구독 등록한 뒤 Order가 배송 중으로 상태를 변경한다. 등록 순서대로 EmailOrderObserver의 update를 호출하고 반환되면 SmsOrderObserver의 update를 호출한다." loading="lazy" decoding="async" />
  <figcaption>이 예제는 이메일 통지가 끝난 뒤 SMS를 호출한다. 첫 호출이 실패하면 다음 통지가 실행되지 않을 수 있다.</figcaption>
</figure>

코드에서는 `subscribe()`로 목록에 넣는 부분, `changeStatus()`로 상태를 바꾸는 부분, 반복문 안의 `update()`를 찾으면 된다. `unsubscribe()`는 더 이상 알림을 받을 필요가 없을 때 목록에서 빼는 기능이다.
...
`Order` 자체가 Subject이며 `subscribe`, `unsubscribe`, 상태 변경 후 통지 책임을 가진다. 호출 흐름은 `changeStatus` → 상태 갱신 → Observer 목록 순회 → 각 `update` 호출 순서다. 예시는 이벤트 데이터를 인자로 보내는 push 방식이다. Observer가 Subject에서 새 상태를 다시 조회하는 pull 방식도 있다.

`List.copyOf(observers)`는 이번 통지에 사용할 목록의 스냅샷을 만든다. 콜백 도중 구독 목록이 바뀌어도 현재 순회를 직접 흔들지 않지만, `Order` 전체를 스레드 안전하게 만드는 것은 아니다. 여러 스레드가 구독과 상태 변경을 동시에 수행한다면 별도의 동기화 정책이 필요하다.

이처럼 호출한 구독자의 처리가 끝날 때까지 기다리는 것을 동기 통지라고 한다. 동기 통지에서 한 Observer가 예외를 던지거나 오래 걸리면 나머지 통지와 주문 처리까지 영향을 받을 수 있다. 실무에서는 Observer별 실패 격리, 재시도, 이벤트 큐 사용 여부를 일관성 요구와 함께 결정해야 한다. 또한 더 이상 통지가 필요 없는 Observer를 `unsubscribe`하지 않으면 원치 않는 알림이 계속되거나 Subject의 참조 때문에 객체 정리가 늦어질 수 있다.

## 프록시 패턴이 해결하는 문제

결제 호출이 어디서 시작되든 권한 검사를 거치게 하고 싶다고 해 보자. 각 호출부에 검사를 반복해서 쓰는 대신, 결제 객체 앞에서 요청을 받는 대리 객체를 둘 수 있다. 프록시는 이 대리 객체를 뜻한다.

여러 호출부가 실제 `PaymentClient`를 바로 사용하면 로깅과 접근 제어가 중복될 수 있다. 이 정책을 외부 결제 통신 코드 안에 섞게 될 수도 있다. 프록시가 실제 클라이언트와 같은 `PaymentClient` 계약을 구현하면 호출부는 둘을 구분하지 않고 `pay` 메서드를 사용한다. 프록시는 요청을 먼저 받아 정책을 적용한 뒤 허용된 호출만 실제 대상에 위임한다.

<figure class="study-diagram study-diagram--pattern">
  <img src="/images/study/design-pattern/payment-proxy.svg" width="480" height="530" alt="호출자가 PaymentClient의 pay를 호출하면 프록시가 로그와 권한 검사를 수행한다. 허용 시 RealPaymentClient에 위임하고, 거부 시 예외로 끝내며 실제 결제를 호출하지 않는다." loading="lazy" decoding="async" />
  <figcaption>프록시도 실제 대상도 PaymentClient를 구현하므로 호출부는 같은 pay() 메서드를 사용한다.</figcaption>
</figure>

아래 코드에서는 프록시의 `pay()` 안에서 `target.pay()`보다 권한 검사가 먼저 실행되는지 확인한다. `target`은 프록시가 대신 호출할 실제 대상이다.
...
여기부터는 다른 분야에서 쓰는 같은 이름을 구분한다. 앞의 Java 예제만 이해하려면 우선 객체 프록시의 호출 흐름까지 확인해도 된다.
...
앞의 프록시 예제에서는 권한 검사와 위임을 직접 작성했다. Spring AOP도 프록시를 통해 여러 객체에 공통 처리를 적용한다. AOP는 관점 지향 프로그래밍의 약자이며, 로깅이나 트랜잭션처럼 여러 기능에 반복되는 처리를 분리하는 데 사용한다. 이런 공통 처리를 횡단 관심사라고 부른다.

클라이언트가 대상 대신 프록시를 호출하면 프록시가 설정된 부가 동작인 advice를 적용하고 실제 대상으로 위임한다. 아래의 JDK·CGLIB 구분은 Spring이 그 대리 객체를 어떤 방식으로 만드는지에 관한 내용이다.

Spring Framework의 일반 선택 규칙은 대상이 인터페이스를 하나 이상 구현하면 **JDK dynamic proxy**, 구현한 인터페이스가 없으면 **CGLIB proxy**를 만드는 것이다. JDK dynamic proxy는 대상이 구현한 인터페이스들을 프록시한다. 따라서 구체 클래스에만 있는 메서드까지 같은 계약으로 사용할 수 있다고 가정하면 안 된다. CGLIB는 대상 타입의 런타임 하위 클래스를 만들어 재정의 가능한 메서드를 가로챈다. `final` 클래스를 프록시할 수 없고 `final`·`private` 메서드에는 advice를 적용할 수 없다.

`proxy-target-class` 같은 설정으로 클래스 기반 프록시를 강제할 수 있다. Spring Boot의 기본값도 구성에 따라 달라질 수 있으므로 실제 운영 설정을 확인해야 한다. 대상 메서드 안의 `this.otherMethod()`는 프록시가 아닌 대상 자신을 호출해 advice를 우회한다. 프록시 기반 AOP의 효과를 기대하는 메서드는 외부에서 프록시 참조를 통해 호출되도록 경계를 설계해야 한다.
