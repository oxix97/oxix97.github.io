...
- SVG 파일 6개가 존재하며 글마다 지정된 이미지, 비어 있지 않은 `alt`,
  캡션이 있는지 검증한다. 설명 문구 자체는 정확한 문자열로 고정하지 않는다.
- MSS·PMTUD·TIME_WAIT의 기술적 정확성은 강의와 IETF 1차 자료를 대조한
  구현 보고서 및 작업별 명세·품질 리뷰에서 검증한다.
